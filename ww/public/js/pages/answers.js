async function renderAnswersPage(eventId) {
  cleanupGame();
  appState.currentView = 'answers';
  ...
  try {
    const res = await API.get(`/game/event/${eventId}/answers?user_id=${appState.user?.id || ''}`);
    if (!res.success) {
      alert(res.message || '加载失败');
      renderFavoritesPage();
      return;
    }
    initAnswersMap(res.data);
  } catch (e) {
    console.error(e);
