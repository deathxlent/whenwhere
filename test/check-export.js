...
const extractPath = path.join(__dirname, 'extract_test');
...
fs.createReadStream(zipPath)
  ...
  .on('close', () => {
    ...
    if (jsonFile) {
      ...
      console.log('\n=== 导出数据结构 ===');
      console.log('地图:', data.maps.map(m => ({ code: m.code, name: m.name })));
      console.log('分类:', data.categories.map(c => ({ code: c.code, name: c.name })));
      console.log('子分类:', data.sub_categories.map(sc => ({
        code: sc.code,
        name: sc.name,
