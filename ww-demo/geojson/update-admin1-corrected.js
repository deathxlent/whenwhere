...
// 读取现有的GeoJSON文件
const rawData = fs.readFileSync('./world_admin1_labels.json', 'utf8');
...
// 移除旧的同名标注，然后添加新的
const countriesToRemove = ['中国', '美国', '日本', '法国', '德国', '意大利', '西班牙', '英国', '韩国', '加拿大', '澳大利亚', '巴西', '印度', '墨西哥', '印度尼西亚', '阿根廷', '波兰', '乌克兰', '土耳其', '沙特阿拉伯', '伊朗', '埃及', '尼日利亚'];

data.features = data.features.filter(f => !countriesToRemove.includes(f.properties.country));

// 添加新的特征
data.features = data.features.concat(newFeatures);

// 写入更新后的数据
fs.writeFileSync('./world_admin1_labels.json', JSON.stringify(data, null, 2), 'utf8');
